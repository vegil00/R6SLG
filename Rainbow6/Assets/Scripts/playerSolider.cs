...
public class playerSolider :soldier
{
  ...
  public  bool scaned;
    ...
    public void aimAtEnemy(enemySolider s)
    {
        GetComponent<SoilderAnimation>().aimAt(s.transform);
        GetComponent<LineRenderer>().positionCount = 0;
        Moved = true;
    }
    public void shootAtEnemy()
    {
        GetComponent<SoilderAnimation>().shootAt();
    }
    public void OnTriggerExit(Collider other)
    {
        if(other.GetComponent<enemySolider>())
        {
            enemySolider solider = other.GetComponent<enemySolider>();
            if (targetList.ContainsKey(solider))
            {
                targetList.Remove(solider);
            }
        }
    }
}
